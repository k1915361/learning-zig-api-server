...
pub fn main() !void {
    ...
    try child.spawn(); // error: FileNotFound //  windowsCreateProcessPathExt (external-command.exe.obj) // .NO_SUCH_FILE => return error.FileNotFound, // spawnWindows (external-command.exe.obj) // return original_err; // spawn (external-command.exe.obj) // return self.spawnWindows(); // main (external-command.exe.obj) // try child.spawn();
